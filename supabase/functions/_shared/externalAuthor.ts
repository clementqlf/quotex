import { searchOpenLibraryAuthors, OpenLibraryAuthor } from './openLibrary.ts';
import { searchAuthorQid, getAuthorWorks, WikidataWork } from './wikidata.ts';
import { searchGoogleBooks, FormattedBook } from './googleBooks.ts';
...
export const searchExternalAuthors = async (query: string): Promise<ExternalAuthor[]> => {
    const results = await searchOpenLibraryAuthors(query);
...
export const getExternalAuthorBooks = async (authorName: string, authorOlid?: string): Promise<FormattedBook[]> => {
    // 1. Resolve Wikidata QID
    let qid = await searchAuthorQid(authorName);
    ...
    const works: WikidataWork[] = await getAuthorWorks(qid);
    console.log(`[ExternalAuthor] Found ${works.length} works for ${authorName}`);

    // 3. Enrich & Format
    const formattedBooks: FormattedBook[] = await Promise.all(works.map(async (work) => {
        let cover = null;
        let isbn = null;
    ...
    const uniqueBooks: FormattedBook[] = [];
