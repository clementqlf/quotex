...
-- Description: Ajout d'index pour résoudre les problèmes de performance
-- ============================================================================
...
CREATE INDEX IF NOT EXISTS idx_userblock_blockedid ON "UserBlock"("blockedId");
...
CREATE INDEX IF NOT EXISTS idx_report_reviewid ON "Report"("reviewId");

-- Index pour UserBook (filtre par status - utilisé dans les requêtes de bibliothèque)
CREATE INDEX IF NOT EXISTS idx_userbook_userid_status ON "UserBook"("userId", status);

-- Index pour Like et UserQuote (requêtes par utilisateur)
CREATE INDEX IF NOT EXISTS idx_like_userid ON "Like"("userId");
CREATE INDEX IF NOT EXISTS idx_userquote_userid ON "UserQuote"("userId");
